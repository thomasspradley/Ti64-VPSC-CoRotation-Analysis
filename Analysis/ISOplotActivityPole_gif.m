...
figure
contourf(r,a_prism(strainStep,:))
colorbar
mtexColorMap black2white
caxis([0,1])
title('prism')

saveas(gcf,[fn 'Prism_' '.png'])
close
...
figure
contourf(r,a_basal(strainStep,:))
colorbar
mtexColorMap black2white
caxis([0,1])
title('basal')

saveas(gcf,[fn 'Basal_' '.png'])
close
...
for i = 1:21
    h = figure;
    set(h, 'visible', 'off');
    filename = [fn 'prismActivity_strain.gif'];
    contourf(r,a_prism(i,:))
    colorbar
    mtexColorMap black2white
    caxis([0,1])
    title(sprintf('Prism Activity at Strain %.0f',i),'visible', 'on');
    set(h, 'visible', 'off');
    drawnow;
    frame = getframe(h);
    im = frame2im(frame);
    ...
    close(h);
    waitbar(i/21, w, sprintf('Prism Progress: %.0f%%',i/21*100) );
end
close(w);
...
for i = 1:21
    h = figure;
    set(h, 'visible', 'off');
    filename = [fn 'basalActivity_strain.gif'];
    contourf(r,a_prism(i,:))
    colorbar
    mtexColorMap black2white
    caxis([0,1])
    title(sprintf('Basal Activity at Strain %.0f',i),'visible', 'on');
    set(h, 'visible', 'off');
    drawnow;
    frame = getframe(h);
    im = frame2im(frame);
    ...
    close(h);
    waitbar(i/21, w, sprintf('Basal Progress: %.0f%%',i/21*100) );
end
close(w);
